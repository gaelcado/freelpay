...
export default function SignContractStep({
  formData,
  updateFormData,
    ...
    <div className="space-y-6">
      <div className="space-y-2">
        <BodyMD className="text-muted-foreground">
          Veuillez lire attentivement le contrat ci-dessous et le signer pour
      ...
      <Card className="border-border/50 overflow-hidden">
        ...
        <CardContent className="p-6 space-y-6">
          <div className="grid grid-cols-2 gap-y-3 text-sm">
            ...
            <div className="font-medium text-right">
              {formData.clientDetails?.name || "Non spécifié"}
            </div>
          </div>
          ...
          <div className="h-48 overflow-y-auto bg-muted/20 p-4 text-sm text-muted-foreground border border-border/30 rounded-md">
            <p className="mb-3 font-semibold text-foreground">
              CONTRAT DE FINANCEMENT DE FACTURE
            </p>
            <p className="mb-3">
              Entre Freelpay, ci-après dénommé "le Financeur", et{" "}
              {formData.userDetails?.firstName || ""}{" "}
              {formData.userDetails?.lastName || ""}, ci-après dénommé "le
              Client".
            </p>
            ...
            <p className="mb-3">
              Le présent contrat a pour objet le financement de la facture n°
              {Math.floor(Math.random() * 10000)} d'un montant de{" "}
              {invoiceAmount.toFixed(2)}€ émise à l'encontre de{" "}
            ...
            <p className="mb-3">
              Le Financeur s'engage à verser au Client la somme de{" "}
              {financedAmount.toFixed(2)}€, correspondant au montant de la
              facture diminué des frais de service de {fees.toFixed(2)}€ et d'un
              taux d'escompte de {(discountRate * 100).toFixed(1)}%.
            </p>
            <p className="mb-2 font-medium text-foreground">
              Article 3 : Modalités de paiement
            </p>
            <p className="mb-3">
              Le versement sera effectué dans un délai de 24 heures suivant la
              signature du présent contrat, par virement bancaire sur le compte
              du Client.
            </p>
            <p className="mb-2 font-medium text-foreground">
            ...
            <p className="mb-3">
              Par la signature du présent contrat, le Client cède au Financeur
              la créance correspondant à la facture mentionnée à l'article 1.
            </p>
            <p className="mb-2 font-medium text-foreground">
              Article 5 : Durée du contrat
            </p>
            <p className="mb-3">
              Le présent contrat prend effet à la date de sa signature et prend
              fin au paiement intégral de la facture par le débiteur.
            </p>
          </div>
        </CardContent>
      </Card>
                    ...
                    <FormItem className="flex flex-row items-start space-x-3 space-y-0 rounded-md p-4 border border-border/50 bg-muted/20">
                      ...
                      <div className="space-y-1 leading-none">
                        <FormLabel className="text-sm font-normal">
                          J'accepte les conditions générales du contrat et je
                          reconnais avoir pris connaissance des modalités de
                          financement.
                        </FormLabel>
                        <FormMessage />
                      </div>
          ...
          <CardContent className="p-5 flex items-start gap-4">
            ...
            <div>
              ...
              <BodyMD className="text-emerald-700 dark:text-emerald-600">
                Votre financement sera disponible sous 24h. Vous recevrez une
              ...
              <div className="mt-4 flex items-center gap-3">
                ...
                <BodySM className="text-emerald-700 dark:text-emerald-600">
                  Traitement en cours - Financement prévu le{" "}
                  {format(
      ...
      <div className="flex items-start gap-3 p-4 bg-blue-50 dark:bg-blue-900/10 rounded-lg border border-blue-100 dark:border-blue-900/20">
        ...
        <div>
          ...
          <BodySM className="text-blue-700 dark:text-blue-600">
            Une copie du contrat signé vous sera envoyée par email à{" "}
            {formData.userDetails?.email || "votre adresse email"}. Vous pourrez
            également le retrouver dans votre espace client.
          </BodySM>
        </div>
      </div>
    </div>
